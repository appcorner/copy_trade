...
string ClientSocket::Receive(string MessageSeparator = "")
{
   ...
   if (TerminalInfoInteger(TERMINAL_X64)) {
      ...
      int res = 1;
      while (res > 0) {
         ArrayInitialize(arrBuffer, 0);
         res = recv(mSocket64, arrBuffer, BufferSize - 1, 0);
         if (res > 0) {
      ...
      while (res > 0) {
         ArrayInitialize(arrBuffer, 0);
         res = recv(mSocket32, arrBuffer, BufferSize - 1, 0);
         if (res > 0) {
            StringAdd(mPendingReceiveData, CharArrayToString(arrBuffer, 0, res));
         } else {
            if (WSAGetLastError() != WSAWOULDBLOCK) mConnected = false;
   ...
   if (mPendingReceiveData == "") {
      ...
      if (idx >= 0) {
         // Skip leading empty messages (e.g. "\n\nDATA\n")
         while (idx == 0) {
         ...
         // After skipping, check if we still have a complete message
         if (idx > 0) {
         ...
         // else: idx == -1, data is incomplete, wait for more
      }
   }
