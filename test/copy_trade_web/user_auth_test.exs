defmodule CopyTradeWeb.UserAuthTest do
  use CopyTradeWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias CopyTrade.Accounts
  alias CopyTrade.Accounts.Scope
  alias CopyTradeWeb.UserAuth

  import CopyTrade.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CopyTradeWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CopyTradeWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CopyTradeWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
